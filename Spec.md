...
- ZIP配布（推奨）: `wpf-ui-agent/`
  ...
  - `.wpf-agent/profiles.json`（ターゲットアプリ定義）
  - `README.md`
...
2. `.wpf-agent/profiles.json` テンプレ生成
...
`.wpf-agent/profiles.json` に複数アプリを登録可能とする。
...
- `.wpf-agent/profiles.json` で複数アプリを登録し、`--profile` で切替できる
